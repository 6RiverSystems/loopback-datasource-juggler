            ...
            physician.patients.create({name: 'a'}, function(err, patient) {
              Address.create({name: 'z'}, function(err, address) {
                ...
                patient.save(function() {
                  physician.patients({include: 'address'}, function(err, p) {
                    if (err) return done(err);

                    p.length.should.equal(1);
                    p[0].name.should.equal('a');
                    p[0].addressId.should.equal(1);
                    p[0].address().id.should.equal(1);
                    p[0].address().name.should.equal('z');
